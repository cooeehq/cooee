...
describe("AI generation contracts", () => {
  ...
  test("normalizes long category ids without regex backtracking", () => {
    const longCategory = `${"reader---visible ".repeat(10_000)} update`;

    expect(normalizeChangelogCategoryId(longCategory)).toBe(
      "reader-visible-reader-visible-reader-visible-rea",
    );
  });
