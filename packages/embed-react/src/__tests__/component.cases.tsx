...
const { CooeeUpdates, buildCooeePoweredByUrl, normalizeShadcnColor } =
  await import("../CooeeUpdates");
...
describe("CooeeUpdates embed", () => {
  test("attributes powered-by links to the embedding hostname", () => {
    expect(buildCooeePoweredByUrl("app.example.com")).toBe(
      "https://cooee.sh/?utm_source=widget&utm_medium=referral&utm_campaign=powered_by&utm_content=app.example.com",
    );
    expect(buildCooeePoweredByUrl()).toBe(
  ...
  test("uses an inset full-viewport popup instead of trigger positioning on mobile", () => {
    const html = renderToStaticMarkup(
      <CooeeUpdates feedUrl="https://cooee.test/feed.json" />,
    );
    const document = domino.createWindow(html).document;
    const styles = requiredElement<HTMLStyleElement>(
      document,
      "style[data-cooee-styles]",
    ).textContent;
    ...
    expect(styles).toContain("block-size: 100% !important");
    expect(styles).toContain("inline-size: 100% !important");
  });
  ...
  test("renders feed images and links the heading to the public changelog", async () => {
    mockFetch(async () =>
      jsonResponse({
        changelog: {
          name: "Cooee",
    ...
    const poweredByLogo = requiredElement<HTMLElement>(
      poweredBy,
    ...
    expect(
      link.querySelector(".cooee-updates__header-title-label")?.textContent,
    ).toBe("Changelog");
    expect(
      link.querySelector(".cooee-updates__header-title-view")?.textContent,
    ).toBe("View");
    expect(
      link.querySelector(".cooee-updates__header-title-icon")?.textContent,
    ).toBe("↗");
    expect(link.getAttribute("aria-label")).toBe("View Changelog");
    expect(link.getAttribute("rel")).toBe("noreferrer");
    expect(link.target).toBe("_blank");
    expect(heading.textContent).toContain("Changelog");
    ...
    expect(poweredBy.textContent).toBe("Powered by");
    expect(poweredByLogo.getAttribute("aria-label")).toBe("cooee");
    expect(poweredByLogo.getAttribute("role")).toBe("img");
    expect(
      poweredByLogo.querySelector(".cooee-updates__powered-by-logo-trigger"),
    ).toBeTruthy();
    expect(poweredByLogo.querySelector("svg")).toBeTruthy();
    expect(poweredByLogo.querySelectorAll(".logo-extra-o").length).toBe(5);

    const logoTrigger = requiredElement<HTMLElement>(
      poweredByLogo,
      ".cooee-updates__powered-by-logo-trigger",
    );
    const logoSvg = requiredElement<SVGElement>(poweredByLogo, "svg");
    expect(logoSvg.hasAttribute("data-play-on-mount")).toBe(false);
    act(() => {
      logoTrigger.dispatchEvent(
        new testWindow.PointerEvent("pointerover", { bubbles: true }),
      );
    });
    expect(logoSvg.getAttribute("data-play-on-mount")).toBe("true");
  });
  ...
  test("opens article entries in an expanded, scrollable popup reader", async () => {
    ...
    const feedPanel = requiredElement<HTMLElement>(
      testWindow.document,
      ".cooee-updates__panel",
    );
    feedPanel.scrollTop = 96;
    ...
    expect(
      panel.querySelector(".cooee-updates__view")?.getAttribute("data-view"),
    ).toBe("article");
    ...
    const articleFooter = requiredElement<HTMLElement>(
      panel,
      ".cooee-updates__footer",
    );
    expect(articleFooter.textContent).toBe("Powered by");
    expect(
    ...
    expect(
      panel.querySelector("[data-cooee-read-more-entry='article-entry']"),
    ).not.toBeNull();
  });
  ...
  test("drops unsafe public links and images", async () => {
    ...
    expect(
      testWindow.document.querySelector(".cooee-updates__header-title-link"),
    ).toBeFalsy();
    expect(footer.textContent).toBe("Powered by");
    expect(
      footer
        .querySelector<HTMLElement>(".cooee-updates__powered-by-logo")
