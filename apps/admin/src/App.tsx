...
type ViewId = "dashboard" | "repositories" | "privacy" | "settings";
...
const backfillActivitySteps = [
  ...
  {
    title: "Grouping related changes",
    detail: "Organising related pull requests into coherent updates.",
    progress: 68,
  },
  {
  ...
  {
    title: "Reviewing generated copy",
    detail: "Checking the drafts for clarity and publication readiness.",
    progress: 88,
  },
  {
...
const releaseBackfillActivitySteps = [
  ...
  {
    title: "Grouping related changes",
    detail: "Organising related release changes into coherent updates.",
    progress: 68,
  },
  {
    title: "Writing draft posts",
...
export function getHistoricalBackfillActionLayout(
  status: HistoricalBackfillStatus,
) {
  if (status === "success") {
    return {
      order: ["run", "close"] as const,
      closeLabel: "Done",
      closeVariant: "default" as const,
      runLabel: "Run again",
      runVariant: "outline" as const,
    };
  }

  return {
    order: ["close", "run"] as const,
    closeLabel: "Cancel",
    closeVariant: "outline" as const,
    runLabel: status === "running" ? "Running backfill" : "Run backfill",
    runVariant: "default" as const,
  };
}
...
export function App({
  ...
  const [regeneratingHeldEntryIds, setRegeneratingHeldEntryIds] = useState<
    Set<string>
  >(() => new Set());
  const [publishingHeldEntryIds, setPublishingHeldEntryIds] = useState<
    Set<string>
  >(() => new Set());
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(
            response,
            "Could not load the held entry count.",
          ),
        );
      }
  ...
  async function loadWorkspaceSettings() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not load settings."),
        );
      }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(
            response,
            "Could not load changelog settings.",
          ),
        );
      }
  ...
  async function saveSettings(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorBody.error ?? "Could not save settings.");
      }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(
            response,
            "Could not save the custom domain.",
          ),
        );
      }
  ...
  async function refreshCustomDomainStatus() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error ?? "Could not refresh the DNS status.");
      }
  ...
  async function loadPublishedEntries(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not load the changelog."),
        );
      }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(
            response,
            "Could not generate the historical changelog. Try again.",
          ),
        );
      }
  ...
  async function signInWithGitHub() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.error ?? body.message ?? "Could not start GitHub sign-in.",
        );
      }
  ...
  async function signOut() {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not sign out. Try again."),
        );
      }
  ...
  async function publishHeldEntry(entry: HeldEntry) {
    ...
    setPublishingHeldEntryIds((ids) => new Set(ids).add(entry.id));
    let regeneratedApiEntry: ApiChangelogEntry | null = null;
    ...
    try {
      ...
      if (!summary) {
        ...
        if (!regenerateResponse.ok) {
          const body = (await regenerateResponse.json().catch(() => ({}))) as {
            error?: string;
          };
          throw new Error(body.error ?? "Could not create the draft.");
        }

        regeneratedApiEntry =
          (await regenerateResponse.json()) as ApiChangelogEntry;
        publishableEntry = toHeldEntry(regeneratedApiEntry);
        summary = publishableEntry.copy.trim();
        if (!summary) {
          throw new Error("The generated draft did not include post copy.");
        }
      }
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not publish this entry."),
        );
      }
  ...
  async function regenerateHeldEntry(entry: HeldEntry) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error ?? "Could not regenerate the draft.");
      }
  ...
  async function performResolveHeldEntry(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not resolve this entry."),
        );
      }
  ...
  async function postManualUpdate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not publish this post."),
        );
      }
  ...
  async function requestPublishedEntryMarketingRegeneration({
    ...
    if (!response.ok) {
      ...
      throw new Error(body.error ?? "Could not regenerate the marketing copy.");
    }
      ...
      if (!response.ok) {
        ...
        const message =
          body.unavailable === true
            ...
            : (body.error ?? "Could not generate the post image.");
  ...
  async function uploadPublishedEntryPostImage(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error ?? "Could not upload the post image.");
      }
  ...
  async function removePublishedEntryPostImage() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error ?? "Could not remove the post image.");
      }
      ...
      if (isPersistedPublishedEntry(entry)) {
        ...
        if (!response.ok) {
          throw new Error(
            await getApiErrorMessage(
              response,
              "Could not delete the changelog.",
            ),
          );
        }
      }
  ...
  async function performMergeSelectedPublishedEntries(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not merge the changelogs."),
        );
      }
      ...
      if (isPersistedPublishedEntry(entry)) {
        ...
        if (!response.ok) {
          throw new Error(
            await getApiErrorMessage(
              response,
              "Could not save the relevance feedback.",
            ),
          );
        }
      }
...
function PublicChangelogFeedRoute({
  ...
  const loadOlderPublicChangelogEntries = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Could not load more updates."),
        );
      }
  ...
  useEffect(() => {
    ...
    async function loadArticle() {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            await getApiErrorMessage(response, "Could not load this article."),
          );
        }
...
function HistoricalBackfillDialog({
  ...
  const canRun =
    ...
    !isRunning;
  const footerActions = getHistoricalBackfillActionLayout(status);
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className="flex max-h-[calc(100vh-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-lg">
        ...
        <DialogFooter className="mx-0 mb-0">
          {footerActions.order.map((action) =>
            action === "run" ? (
              <Button
                disabled={!canRun}
                key="run"
                onClick={onRun}
                type="button"
                variant={footerActions.runVariant}
              >
                {isRunning ? (
                  <LoaderCircleIcon
                    aria-hidden
              ...
              <Button
                ...
                type="button"
                variant={footerActions.closeVariant}
              >
                {footerActions.closeLabel}
              </Button>
            ),
          )}
        </DialogFooter>
        ...
        {githubConnection.repositories.length > 0 ? (
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            ...
            <div className="text-sm leading-6 text-muted-foreground">
              {selectedRepositoryCount} connected
            </div>
          </div>
        ) : null}
...
function SettingsView({
  ...
  async function uploadLogo(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error ?? "Could not upload the logo.");
      }
  ...
  async function removeLogo() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.error ?? "Could not remove the logo.");
      }
  ...
  async function uploadCustomBrandAsset(
    ...
    const file = event.target.files?.[0];
    if (!file || customBrandAssetStatus) return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.error ?? `Could not upload the ${label.toLowerCase()}.`,
        );
      }
  ...
  async function removeCustomBrandAsset(kind: CustomBrandAssetKind) {
    if (customBrandAssetStatus) return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.error ?? `Could not remove the ${label.toLowerCase()}.`,
        );
      }
      ...
      <div className="grid gap-8 xl:grid-cols-[12rem_minmax(0,1fr)] xl:items-start xl:gap-10">
        ...
        <div className="min-w-0 space-y-10">
          ...
          >
            ...
            <SettingsRow
              description="Shown on light backgrounds. Falls back to the default logo when empty. PNG, JPEG, WebP, GIF, or SVG up to 512 KB."
              htmlFor={
            ...
            <SettingsRow
              description="Used in browser tabs for the public changelog. PNG, SVG, or ICO up to 256 KB."
              htmlFor={
          ...
          >
            ...
            {postImageAiUiEnabled ? (
              <SettingsRow
                description="Brand cards render locally. Reference and Illustration use the configured AI image model."
                title="Image mode"
              >
                <RadioGroup
