...
describe("Cooee admin app", () => {
  ...
  test("offers explicit learning-backed resolutions for held drafts", () => {
    ...
    expect(html).toContain("Create &amp; publish post");
  ...
  test("shows API error details without exposing HTTP status numbers", async () => {
    await expect(
      getApiErrorMessage(
        Response.json(
        ...
        "Could not generate the historical changelog.",
      ),
    ).resolves.toBe("Generation is temporarily unavailable.");
    ...
    ).resolves.toBe("Could not generate the historical changelog.");
  });

  test("makes completion primary before offering another backfill", () => {
    expect(getHistoricalBackfillActionLayout("success")).toEqual({
      order: ["run", "close"],
      closeLabel: "Done",
    ...
    expect(getHistoricalBackfillActionLayout("running")).toEqual({
      order: ["close", "run"],
      closeLabel: "Cancel",
      closeVariant: "outline",
      runLabel: "Running backfill",
      runVariant: "default",
    });
  });
