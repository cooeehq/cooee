      ...
      try {
        const response = await (async () => {
          ...
          if (
            ...
            const workspaceId = getWorkspaceId(url);
            const changelogs = await store.listChangelogs(workspaceId);
            const changelog = changelogs.length === 1 ? changelogs[0] : null;
            if (!changelog) {
              if (changelogs.length > 1) {
                return json(
...
async function selectRepositoryForChangelog({
  ...
  if (!changelog) {
    throw json(
      { error: "The changelog could not be created." },
      { status: 409 },
    );
  }
  const installations = await store.listGitHubInstallations(workspaceId);
...
async function connectCliSetupSessionRepository({
  ...
  try {
    ...
    await store.updateCliSetupSession({
      ...
      status: "repository-not-granted",
      error:
        "Cooee could not connect this repository. Check GitHub App access.",
    });
  }
}
